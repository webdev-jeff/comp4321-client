  ...
  render() {
    return (
      <div className="row border-top">
        <div className="col-2">
          <p>source</p>
          <button className="btn btn-primary" onClick={this.searchSimilar}>Get similar pages</button>
        </div>
...
class Main extends Component {
  constructor(props) {
    ...
    this.handleSearchAllKeywordsSelected = this.handleSearchAllKeywordsSelected.bind(this);
    this.handleSearch = this.handleSearch.bind(this);
    this.setQuery = this.setQuery.bind(this);
    this.toggleModal = this.toggleModal.bind(this);
    this.handleSearchInSearchHistory = this.handleSearchInSearchHistory.bind(this);
    // this.handleSearchInAllKeywords = this.handleSearchInAllKeywords.bind(this);
    this.onSearchHistoryCheckboxBtnClick = this.onSearchHistoryCheckboxBtnClick.bind(this);
    this.onAllKeywordsCheckboxBtnClick = this.onAllKeywordsCheckboxBtnClick.bind(this);
  ...
  handleResearch(event) {
    let query = this.state.searchHistorySelected.map((idx) => this.state.history_queries[idx]).join(' ');
    this.setQuery(query);
    this.toggleModal();
    if (typeof event !== 'undefined') event.preventDefault();
    this.handleSearch(event);
  ...
  handleSearchAllKeywordsSelected(event) {
    let query = this.state.allKeywordsSelected.map((idx) => this.state.all_keywords[idx]).join(' ');
    this.setQuery(query);
    if (typeof event !== 'undefined') event.preventDefault();
    this.handleSearch(event);
  }
  ...
  handleSearchInSearchHistory() {
    let searchBarValue = document.getElementById("searchBarInSearchHistory").value;
    if (searchBarValue === "") {
      let newMask = this.state.history_queries.map(() => true);
      this.setState({ history_queries_mask: newMask });
    } else {
      let newMask = this.state.history_queries.map((query) => query.includes(searchBarValue));
      this.setState({ history_queries_mask: newMask });
    }
  }
  ...
  onSearchHistoryCheckboxBtnClick(selected) {
    ...
    this.setState({ searchHistorySelected: [...this.state.searchHistorySelected] });
  }
    ...
    return (
      <div className="container-fluid ml-0 mr-0" style={{ height: "100vh" }}>
        <div className="row" style={{ height: "100%" }}>
          <div className="col-10" style={{ overflowY: "scroll", height: "100%" }}>
              ...
              <div className="col-3">
                ...
                <Modal isOpen={this.state.isModalOpen} toggle={this.toggleModal}>
                  ...
                  <ModalBody>
                    <Row>
                      <Col>
                        <Input onChange={this.handleSearchInSearchHistory}
                  ...
                  <ModalFooter>
                    <Button className="ml-5" onClick={this.handleResearch}>
                      {this.state.searchHistorySelected.length <= 1 ? "Search selected" : "Combine and search selected"}
                    </Button>
                  </ModalFooter>
                </Modal>
              </div>
            </div>
            {this.state.result.map((r) => <RenderSearchResult key={r._id} result={r} handleSearch={this.handleSearch} setQuery={this.setQuery} />)}
          </div>
          <div className="col-2 border-left" style={{ height: "100%" }}>
            <p>Search and select from all stemmed keywords in our database</p>
            <Button onClick={this.handleSearchAllKeywordsSelected} style={{ width: "100%", marginBottom: "15px" }}>
              {this.state.allKeywordsSelected.length <= 1 ? "Search selected" : "Combine and search selected"}
            </Button>
